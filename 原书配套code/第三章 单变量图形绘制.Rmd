...
当想要显示随时间变化的趋势时，但不关心显示确切的值，面积图是一个很好的选择。以下示例显示了1880年至2015年间美国婴儿名字频率的演变。
```{r}
# 加载包
library(tidyverse)
library(babynames)
...
# babynames 是babynames包中的数据集，记录了不同年份的婴儿的名称数量
data <- babynames %>% 
...
在绘制密度图的过程中需要注意的是，分组顺序（从下到上）可能会对于图形产生影响，请多尝试几个顺序，然后选择合适的分组顺序。
...
```{r}
    ggplot(sunspotyear, aes(x = Year, y = Sunspots)) +
geom_area(colour = "black",fill ='red',linetype=2) # 设置了颜色，线条种类
```
...
在很多分析的时候，想要了解的是不同情况下的面积图。这个时候可以将不同情况的面积图分别绘制出来，绘制多张面积图。但是，很好的绘图方法是通过堆叠的方式，将多张面积图会旨在同一张统计图表中。
构建堆叠的面积图的输入的数据框需要3列：

1. x：用于X轴的数字变量，通常是一个时间。
2. y：用于Y轴的数字变量。关心的变量
3. group：每组将完成一个形状。
...
# create data
time <- as.numeric(rep(seq(1,7),each=7))  # x 轴的数据
value <- runif(49, 10, 100)               # y 轴的数据
group <- rep(LETTERS[1:7],times=7)        # 分组，每组一个形状
...
ggplot(data, aes(x=time, y=value, fill=group)) + 
    geom_area()
```
上面的代码使用自己构建的数据集创建了一副堆叠的面积图。如果希望控制堆叠的顺序，可以使用factor()函数给出特定的顺序。
```{r}
# 给变量一个特定的顺序
...
```
上面的代码首先对group 的因子顺序进行了设置，然后再绘制堆叠的面积图，通过这样的方式对图形的堆叠顺序进行了调整。

这里使用的是gcookbook保重的uspopage数据集记性绘制堆叠的面积图。这个数据及是美国1900年到2002年的人口普查的数据。数据集中第一个特征Year表示年份，第二个特征AgeGroup 表示年龄段，第三个特征Thousands表示人数。通过堆叠图的方式可以会出不同年份，不同的年龄段的人口人数。
```{r,message=FALSE,warning=FALSE}
require(gcookbook)# 加载包
require(tidyverse)

data("uspopage") # 加载数据
uspopage %>% head(3) # 查看数据的前三行

ggplot(uspopage, aes(x = Year, y = Thousands, fill = AgeGroup)) +
      geom_area(colour ='black') # 绘制图形
```
...
```{r}
 ggplot(uspopage, aes(x = Year, y = Thousands, fill = AgeGroup)) +
      ...
      scale_fill_brewer(palette = "Pastel1") # 绘制图形
```
...
geom_area(colour = NA, alpha = .4) + scale_fill_brewer(palette = "Pastel1") + geom_line(position = "stack", size = .2) # 绘制图形
```
...
```{r}
a + geom_density(aes(fill = Species,color = Species)) # 绘图 设置了fill和color参数
```
...
```{r}
a + geom_density(aes(fill = Species)) +scale_fill_manual(values=c("red", "green","black"))+ theme_minimal() # 绘图，调整颜色
```
...
```{r}
a + geom_density(aes(fill = Species,color = Species)) +scale_fill_brewer(palette = "BrBG")+ theme_minimal()# 绘图，调整颜色
```
...
```{r}
a + geom_density(aes(fill = Species)) +scale_fill_excel() + theme_minimal()# 绘图，调整颜色


a + geom_density(aes(fill = Species)) +scale_fill_economist() + theme_minimal()# 绘图，调整颜色
...
```{r}
a + geom_density(aes(color = Species)) +scale_color_manual(values=c("red", "green","black"))+ theme_minimal()# 绘图，调整颜色
```
...
ggplot(diamonds, aes(x = price, y = color,fill = color)) +
  geom_density_ridges() +
  theme_ridges() + 
  theme(legend.position = "none",axis.title.y= element_text(hjust = 0,vjust = 1))
```
...
library("ggridges") # 加载包
library("tidyverse")
OrchardSprays_data <- OrchardSprays# 获取数据

ggplot(OrchardSprays_data, aes(x=decrease,y=treatment,fill=treatment))+ # 绘制图形
  ...
  theme(plot.title = element_text(hjust = 0.5,size = 10),axis.title.y=element_text(hjust = 0,vjust = 0))
```
...
```{r}
a + geom_density(aes(fill = Species,color = Species)) +scale_colour_economist() + theme_minimal() # 绘图


a + geom_density(aes(fill = Species,color = Species,alpha=0.4)) +scale_colour_economist() + theme_minimal() # 绘图
```
...
```{r}
ggplot(data=diamonds, aes(x=price, group=color, fill=color)) + # 绘图
...
```{r}
data=data.frame(value=rnorm(100)) # 创建数据集
...
```{r}
a <- ggplot(data = iris,aes(Sepal.Length)) # 绘图
...
```{r}
data <- data.frame( # 创建数据集
  var1 = rnorm(1000),
  var2 = rnorm(1000, mean=2)
)
p <- ggplot(data, aes(x=x) ) + # 绘制图形
...
p <- ggplot(iris, aes(x=Sepal.Width)) + geom_histogram(aes(y = ..density.., 
            fill = ..count..), binwidth=0.2) + geom_density()   # 绘图
p
```
